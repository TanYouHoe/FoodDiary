...
| 2 | **Stored XSS on the map.** Restaurant name, cuisine and address go into the info window as raw HTML. | `src/ui/lists.js` `infoWindowHtml` | Fixed — fix(web): map XSS, suggestion cards, custom dish types, nested form, object URLs |
| 3 | **Dashboard suggestion cards lose their data.** The page keeps only name, restaurant, cuisine, dishes and `reason`. The badge (`suggestion_type`), price, stars and the server's `explanation` are dropped, and `reason` does not exist. The cards show a name and a cuisine only. | `src/ui/suggestions.js` `toMealSuggestions` | Fixed — fix(web): map XSS, suggestion cards, custom dish types, nested form, object URLs |
| 4 | **Meal period and weekday depend on the server's time zone.** The browser sends UTC; the server reads local hours. On a UTC host a Malaysian lunch is breakfast. | `logic/meal-period.js` | Fixed — fix: meal periods follow the user's time zone. Note: meals stored earlier with a zone-less `visited_at` (no `Z` or offset) are converted to UTC once, at database open, reading them in `DEFAULT_TIME_ZONE`. |
| 5 | **The log-a-meal date defaults to the UTC day** while the time defaults to local time. Between midnight and 08:00 in Malaysia the date is yesterday. | `src/ui/format.js` `utcDateInput` | Fixed — fix: meal periods follow the user's time zone |
...
| 7 | **Custom dish types are invisible.** The server categorises a dish as a custom type (e.g. `curry`); the dish chips show only the nine built-in categories, so those dishes disappear from the dish editor and the meal detail. | `src/ui/dishes.js` `groupDishes` | Fixed — fix(web): map XSS, suggestion cards, custom dish types, nested form, object URLs |
