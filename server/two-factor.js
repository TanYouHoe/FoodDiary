...
// design, so a hash would make codes impossible to check.
...
export function makeTwoFactor(db) {
  ...
  const clearFactor = db.prepare(`UPDATE users SET totp_secret = NULL, totp_pending_secret = NULL, totp_enabled_at = NULL,
    totp_last_step = NULL, token_version = token_version + 1 WHERE id = ?`);
  ...
  const replaceCodes = (userId) => {
    ...
    for (const code of codes) insertCode.run(userId, hashBackupCode(code));
    return codes;
  };
  ...
  return {
    ...
    useBackupCode(userId, input, now) {
      const code = normaliseBackupCode(input);
      return code !== null && spendCode.run(now.toISOString(), userId, hashBackupCode(code)).changes === 1;
    },
    ...
    // Removes the factor, pending secret and backup codes; the token version goes up.
    clear: db.transaction((userId) => {
      clearFactor.run(userId);
      deleteCodes.run(userId);
    }),
