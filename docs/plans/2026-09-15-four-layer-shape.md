...
| 4 | **Meal period and weekday depend on the server's time zone.** The browser sends UTC; the server reads local hours. On a UTC host a Malaysian lunch is breakfast. | `logic/meal-period.js` | Fixed — fix: meal periods follow the user's time zone |
