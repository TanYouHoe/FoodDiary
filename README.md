...
| ----------------------- | ------- | ----------------------- | -------- | ---------------------------------------------------------------------- |
| `HOST`                  | server  | `127.0.0.1`             | `127.0.0.1` | Listen address. The default reaches this machine only; cloudflared on the same machine dials it. An IP address or host name, else the server refuses to start. IPv6 zone IDs (`fe80::1%eth0`) and IPv4-mapped forms (`::ffff:127.0.0.1`) are not accepted. |
...
| `TRUST_PROXY`           | server  | _(unset: no proxy)_     | `loopback` | Proxies trusted to name the client IP in `X-Forwarded-For`, so the sign-in lockout counts the real IP: `loopback` (a proxy on this machine, such as cloudflared), a hop count from 1 to 3, or a comma-separated IP/CIDR list (`loopback` cannot be mixed with IPs). `true` and anything else stop the server: any client could then pick its own IP. **In production with a loopback `HOST` (the default) it is required**, else every client shares the proxy's IP in the lockout. |
...
- **Updates** — a new deploy does not replace the running version by itself. The app looks for a new `sw.js` on load, every hour, when the page becomes visible again, and at once when an API answer names a different build. Every build writes its id into the app script and `dist/build-id.txt`; the server reads that file at start-up and sends it as `X-App-Build` on API answers (no file, no header). When a new worker waits, a banner says *A new version of Food Diary is ready.* **Reload** switches to it and reloads the page; **Later** hides the banner, and it comes back at the next check while the new version still waits. The banner is registered from the app's own script (`src/hooks/useAppUpdate.js`), never an inline script, so the CSP stays `script-src 'self'`.
- **What is cached** — the built app shell (scripts, styles, `index.html`, the manifest and the icons), so the app opens offline; page navigations fall back to the cached `index.html`. Photos under `/uploads/` are cached first-served (cache `photos`, at most 200, for 30 days, only `200` answers — never a 404, 403 or opaque answer). The photo cache is deleted at sign-out and when a stored session is refused.
